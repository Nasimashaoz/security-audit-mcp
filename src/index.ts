...
server.tool(
  "cve_lookup",
  "Lookup vulnerability details from the official MITRE CVE API",
  {
    cveId: z.string().describe("CVE ID e.g. 'CVE-2021-34527'"),
  },
  async ({ cveId }) => {
    try {
      ...
      if (!response.ok) {
        return {
          content: [{ type: "text", text: `Failed to fetch ${cveId}: HTTP ${response.status}` }],
          isError: true,
        };
      }
      ...
      return {
        content: [{ type: "text", text: `Error fetching ${cveId}: ${(err as Error).message}` }],
        isError: true,
      };
    }
  }
