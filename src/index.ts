...
server.tool(
  "cve_lookup",
  "Look up details for a specific Common Vulnerabilities and Exposures (CVE) ID using the official MITRE API",
  {
  ...
  async ({ cveId }) => {
    try {
      ...
      return {
        content: [{ type: "text", text: `Failed to look up CVE '${cveId}': ${(error as Error).message}` }],
        isError: true,
      };
    }
  }
