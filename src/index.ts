...
const FrameworkEnum = z.enum(Object.keys(FRAMEWORKS) as [string, ...string[]]);
...
server.tool(
  ...
  async ({ cveId }) => {
    try {
      ...
      return {
        content: [{ type: "text", text: `Error fetching CVE '${cveId}': ${error instanceof Error ? error.message : String(error)}` }],
        isError: true,
      };
    }
  }
