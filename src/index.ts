...
server.tool(
  "cve_lookup",
  "Lookup details for a specific CVE from the official MITRE API",
  {
  ...
  async ({ cveId }) => {
    try {
      ...
      const metrics = cna?.metrics?.[0]?.cvssV3_1 || cna?.metrics?.[0]?.cvssV3_0 || cna?.metrics?.[0]?.cvssV2_0;
      let severityStr = "Unknown";
      if (metrics) {
         severityStr = `Score: ${metrics.baseScore}, Severity: ${metrics.baseSeverity || "N/A"}, Vector: ${metrics.vectorString}`;
      }
      ...
      return {
        content: [{
          type: "text",
          text: JSON.stringify(cveDetails, null, 2),
        }],
      };
    } catch (error: any) {
      return {
        content: [{ type: "text", text: `Failed to lookup CVE: ${error.message}` }],
        isError: true,
      };
    }
  }
