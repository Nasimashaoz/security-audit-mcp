...
server.tool(
  ...
  "Search for security controls by keyword across all frameworks",
  {
    ...
    framework: z.enum(["all", ...Object.keys(FRAMEWORKS)] as [string, ...string[]]).default("all"),
  },
...
server.tool(
  "cve_lookup",
  "Lookup vulnerability details by CVE ID using the official MITRE API",
  {
  ...
  async ({ cveId }) => {
    try {
      ...
      if (!response.ok) {
        return {
          content: [{ type: "text", text: `Error fetching CVE details: ${response.statusText}` }],
          isError: true,
        };
      }
      ...
      return {
        content: [{ type: "text", text: `Failed to fetch CVE: ${error?.message || String(error)}` }],
        isError: true,
      };
    }
  }
