...
server.tool(
  ...
  "Search for security controls by keyword across all frameworks",
  {
    ...
    framework: z.enum(Object.keys(FRAMEWORKS).concat("all") as [string, ...string[]]).default("all"),
  },
...
server.tool(
  "cve_lookup",
  "Look up a CVE by ID using the MITRE CVE API",
  {
  ...
  async ({ cveId }) => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`MITRE API returned ${response.status}`);
      }
      ...
      return {
        content: [{ type: "text", text: `Error fetching CVE '${cveId}': ${err instanceof Error ? err.message : String(err)}` }],
        isError: true,
      };
    }
  }
