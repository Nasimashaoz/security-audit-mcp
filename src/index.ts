...
server.tool(
  ...
  async ({ cveId }) => {
    try {
      ...
      let cvssScore = "N/A";
      let cvssSeverity = "N/A";

      if (metrics.length > 0) {
        const metric = metrics[0];
        const cvssData = metric.cvssV3_1 || metric.cvssV3_0 || metric.cvssV2_0;
        if (cvssData) {
          cvssScore = cvssData.baseScore?.toString() || "N/A";
          cvssSeverity = cvssData.baseSeverity || "N/A";
        }
      }
      ...
      return {
        content: [{ type: "text", text: `Failed to fetch CVE data: ${error.message}` }],
        isError: true,
      };
    }
  }
