  ...
  async ({ cveId }) => {
    try {
      ...
      if (!response.ok) {
        ...
        return {
          content: [{ type: "text", text: `Failed to fetch CVE '${cveId}': HTTP ${response.status}` }],
          isError: true,
        };
      }
      ...
      const metricsInfo = metrics.map((m: any) => {
        const cvss31 = m.cvssV3_1;
        if (cvss31) {
          return `CVSS v3.1: ${cvss31.baseScore} (${cvss31.baseSeverity})`;
        }
        const cvss30 = m.cvssV3_0;
        if (cvss30) {
          return `CVSS v3.0: ${cvss30.baseScore} (${cvss30.baseSeverity})`;
        }
        const cvss2 = m.cvssV2_0;
        if (cvss2) {
          return `CVSS v2.0: ${cvss2.baseScore}`;
        }
      ...
      const result = {
        ...
        state: data?.cveMetadata?.state || "UNKNOWN",
        description: englishDesc,
        metrics: metricsInfo || "No metrics available.",
        datePublished: data?.cveMetadata?.datePublished || "Unknown",
        dateUpdated: data?.cveMetadata?.dateUpdated || "Unknown",
      };
