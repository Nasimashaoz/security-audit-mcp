...
export const server = new McpServer({
  ...
  version: "1.0.0",
  description: "AI-powered security audit tools for multiple compliance frameworks",
});

// Extract typed keys dynamically from FRAMEWORKS so we don't hardcode them
...
server.tool(
  "cve_lookup",
  "Lookup details for a specific CVE from the MITRE CVE API",
  {
  ...
  async ({ cveId }) => {
    try {
      ...
      return {
        content: [{ type: "text", text: `Error fetching CVE data: ${error instanceof Error ? error.message : String(error)}` }],
        isError: true,
      };
    }
  }
