...
server.tool(
  "cve_lookup",
  "Look up a CVE (Common Vulnerabilities and Exposures) from MITRE",
  {
  ...
  async ({ cveId }) => {
    try {
      ...
      if (!response.ok) {
        return {
          content: [{ type: "text", text: `Failed to lookup ${cveId}: ${response.status} ${response.statusText}` }],
          isError: true,
        };
      }
      ...
      return {
        content: [{ type: "text", text: `Error looking up ${cveId}: ${error instanceof Error ? error.message : String(error)}` }],
        isError: true,
      };
    }
  }
