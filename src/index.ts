...
server.tool(
  ...
  async ({ cveId }) => {
    try {
      ...
      if (data.containers?.cna?.metrics && data.containers.cna.metrics.length > 0) {
        const metrics = data.containers.cna.metrics[0];
        if (metrics.cvssV3_1) {
          severity = `${metrics.cvssV3_1.baseSeverity} (${metrics.cvssV3_1.baseScore})`;
        } else if (metrics.cvssV3_0) {
          severity = `${metrics.cvssV3_0.baseSeverity} (${metrics.cvssV3_0.baseScore})`;
        } else if (metrics.cvssV2_0) {
          severity = `${metrics.cvssV2_0.baseSeverity} (${metrics.cvssV2_0.baseScore})`;
        }
      }
