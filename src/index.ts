...
export async function getFrameworkHandler({ framework }: { framework: string }) {
...
export async function auditItemHandler({ sessionId, framework, itemId, status, notes }: { sessionId: string, framework: string, itemId: string, status: "pass" | "fail" | "skip", notes?: string }) {
  const fw = FRAMEWORKS[framework];
...
export async function generateReportHandler({ sessionId, format }: { sessionId: string, format: "json" | "markdown" | "html" }) {
  const session = sessions.get(sessionId);
  if (!session) {
    return {
...
export async function getRiskSummaryHandler({ framework }: { framework: string }) {
  const fw = FRAMEWORKS[framework];
...
export async function searchControlsHandler({ query, framework }: { query: string, framework: string }) {
...
export async function cveLookupHandler({ cveId }: { cveId: string }) {
  try {
    const response = await fetch(`https://cveawg.mitre.org/api/cve/${encodeURIComponent(cveId)}`);
    if (!response.ok) {
    ...
    return {
      content: [{ type: "text" as const, text: `Error looking up CVE: ${error.message}` }],
      isError: true,
    };
  }
}
...
server.tool(
  "cve_lookup",
  "Look up details for a specific CVE from the MITRE API",
  {
