...
export const server = new McpServer({
  ...
  version: "1.0.0",
  description: "AI-powered security audit tools for various frameworks including OWASP, NIST, ISO 27001, PCI-DSS, and more.",
});
...
server.tool(
  ...
  async ({ framework }) => handleGetFramework({ framework })
);
...
server.tool(
  ...
  async (args) => handleAuditItem(args)
);
...
server.tool(
  ...
  async (args) => handleGenerateReport(args)
);
...
server.tool(
  ...
  async ({ framework }) => handleGetRiskSummary({ framework })
);
...
server.tool(
  ...
  async (args) => handleCveLookup(args)
);
