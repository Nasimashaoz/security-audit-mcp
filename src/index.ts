...
server.tool(
  "cve_lookup",
  "Look up a CVE by ID using the official MITRE API",
  {
  ...
  async ({ cveId }) => {
    try {
      ...
      // Basic extraction from the complex MITRE format
      const cveData = data.containers?.cna;
      const title = cveData?.title || cveId;
      const description = cveData?.descriptions?.[0]?.value || "No description available";
      const metrics = cveData?.metrics?.[0]?.cvssV3_1 || cveData?.metrics?.[0]?.cvssV3_0;

      const summary = {
        id: cveId,
        title,
        description,
        cvss: metrics ? { score: metrics.baseScore, severity: metrics.baseSeverity } : null,
        status: data.cveMetadata?.state || "UNKNOWN",
        published: data.cveMetadata?.datePublished || "UNKNOWN",
        updated: data.cveMetadata?.dateUpdated || "UNKNOWN"
      };
      ...
      return {
        content: [{ type: "text", text: `Error fetching CVE '${cveId}': ${err.message}` }],
        isError: true,
      };
    }
  }
