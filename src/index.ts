...
server.tool(
  "cve_lookup",
  "Look up a Common Vulnerabilities and Exposures (CVE) ID using the MITRE API",
  {
    cveId: z.string().regex(/^CVE-\d{4}-\d{4,}$/i).describe("The CVE ID to look up, e.g., CVE-2021-44228"),
  },
  async ({ cveId }) => {
    try {
      const response = await fetch(`https://cveawg.mitre.org/api/cve/${cveId.toUpperCase()}`);
      if (!response.ok) {
