...
server.tool(
  "cve_lookup",
  "Fetch detailed vulnerability information for a specific CVE ID from the official MITRE API",
  {
  ...
  async ({ cveId }) => {
    try {
      ...
      return {
        content: [{ type: "text", text: `Error fetching CVE data: ${(error as Error).message}` }],
        isError: true,
      };
    }
  }
