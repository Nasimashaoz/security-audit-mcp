...
export async function handleCveLookup(args: { cveId: string }) {
  try {
    const url = `https://cveawg.mitre.org/api/cve/${encodeURIComponent(args.cveId)}`;
    ...
    return {
      content: [{ type: "text" as const, text: `Network error or failed to parse response: ${err.message}` }],
      isError: true,
    };
  }
}
