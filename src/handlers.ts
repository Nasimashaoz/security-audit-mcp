...
// In-memory session storage exported for testing purposes if needed
...
export async function handleCveLookup(args: { cveId: string }) {
  try {
    const encodedId = encodeURIComponent(args.cveId);
...
export async function handleGenerateReport(args: { sessionId: string, format: "json" | "markdown" | "html" }) {
  ...
  if (!fw) {
      return {
          content: [{ type: "text" as const, text: `Framework '${session.framework}' not found for session.` }],
          isError: true,
      };
  }
