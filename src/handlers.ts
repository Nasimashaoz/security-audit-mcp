...
import type { AuditSession, FrameworkItem, AuditResult } from "./types.js";
...
export async function handleCveLookup(args: { cveId: string }) {
  const { cveId } = args;
  const sanitizedCveId = encodeURIComponent(cveId.trim());
  ...
  try {
    ...
    if (!response.ok) {
      return {
        content: [{
          type: "text" as const,
          text: `Failed to fetch CVE data for ${cveId}: HTTP ${response.status}`,
        }],
        isError: true,
      };
    }
    ...
    if (data.error) {
       return {
        content: [{
          type: "text" as const,
          text: `CVE data for ${cveId} not found or error returned from MITRE API: ${data.error}`,
        }],
        isError: true,
      };
    }
    ...
    const title = containers.title || "No title available";
    const descriptions = containers.descriptions || [];
    const description = descriptions.length > 0 ? descriptions[0].value : "No description available";
    const state = data.cveMetadata?.state || "UNKNOWN";

    // Attempt to extract metrics (CVSS)
    let cvss = null;
    ...
    const formattedData = {
      cveId: data.cveMetadata?.cveId || cveId,
      state,
      title,
      description,
      cvss: cvss ? {
      ...
      mitreUrl: `https://cveawg.mitre.org/api/cve/${sanitizedCveId}`,
    };
    ...
    return {
      content: [{
        type: "text" as const,
        text: `Error fetching CVE data for ${cveId}: ${msg}`,
      }],
      isError: true,
    };
  }
...
export async function handleAuditItem(args: {
  ...
  const result: AuditResult = { itemId, title: item.title, risk: item.risk, status, notes: notes ?? "" };
  if (existing >= 0) {
