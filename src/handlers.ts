...
export async function handleAuditItem(args: { sessionId: string; framework: string; itemId: string; status: "pass" | "fail" | "skip"; notes?: string }, sessions: Map<string, AuditSession>) {
...
export async function handleGenerateReport(args: { sessionId: string; format?: "json" | "markdown" | "html" }, sessions: Map<string, AuditSession>) {
...
export async function handleSearchControls(args: { query: string; framework?: string }) {
  const { query, framework = "all" } = args;
...
export async function handleCveLookup(args: { cveId: string }) {
  ...
  if (!/^CVE-\d{4}-\d{4,}$/.test(cveId)) {
    return {
      content: [{ type: "text" as const, text: `Invalid CVE ID format: '${cveId}'. Expected format: CVE-YYYY-NNNNN` }],
      isError: true,
    };
  }
  ...
  try {
    ...
    if (!response.ok) {
        if (response.status === 404) {
             return {
                content: [{ type: "text" as const, text: `CVE '${cveId}' not found in MITRE database.` }],
                isError: true,
              };
        }
