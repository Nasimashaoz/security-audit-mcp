...
export async function handleCveLookup(args: { cveId: string }) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return {
        content: [{ type: "text" as const, text: `Failed to fetch CVE '${cveId}'. Status: ${response.status}` }],
        isError: true,
      };
    }
    ...
    return {
      content: [{ type: "text" as const, text: `Error fetching CVE '${cveId}': ${error?.message || String(error)}` }],
      isError: true,
    };
  }
}
