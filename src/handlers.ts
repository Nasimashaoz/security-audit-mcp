...
export async function handleCveLookup(args: { cveId: string }) {
  try {
    const sanitizedId = encodeURIComponent(args.cveId);
    const response = await fetch(`https://cveawg.mitre.org/api/cve/${sanitizedId}`);
    if (!response.ok) {
      return {
        content: [{ type: "text" as const, text: `CVE '${args.cveId}' not found or API error.` }],
        isError: true,
      };
    }
    ...
    return {
      content: [{ type: "text" as const, text: `Error fetching CVE '${args.cveId}': ${String(err)}` }],
      isError: true,
    };
  }
}
