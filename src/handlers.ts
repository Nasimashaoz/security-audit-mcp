...
import { sessions, getSession, setSession, hasSession } from "./state.js";
import type { FrameworkItem, AuditSession } from "./types.js";
...
export async function handleGetFramework({ framework }: { framework: string }) {
...
export async function handleAuditItem({ sessionId, framework, itemId, status, notes }: { sessionId: string; framework: string; itemId: string; status: "pass" | "fail" | "skip"; notes?: string }) {
  const fw = FRAMEWORKS[framework];
  if (!fw) {
      return {
        content: [{ type: "text" as const, text: `Framework '${framework}' not found.` }],
  ...
  if (!hasSession(sessionId)) {
    setSession(sessionId, { id: sessionId, framework, results: [], startedAt: new Date().toISOString() });
  }
...
export async function handleGenerateReport({ sessionId, format }: { sessionId: string; format: string }) {
  const session = getSession(sessionId);
  if (!session) {
    return {
...
export async function handleGetRiskSummary({ framework }: { framework: string }) {
  const fw = FRAMEWORKS[framework];
  if (!fw) {
      return {
        content: [{ type: "text" as const, text: `Framework '${framework}' not found.` }],
...
export async function handleSearchControls({ query, framework }: { query: string; framework: string }) {
...
export async function handleCveLookup({ cveId }: { cveId: string }) {
  try {
    const encodedId = encodeURIComponent(cveId);
    const response = await fetch(`https://cveawg.mitre.org/api/cve/${encodedId}`);

    if (!response.ok) {
      if (response.status === 404) {
        return {
          content: [{ type: "text" as const, text: `CVE '${cveId}' not found.` }],
          isError: true,
        };
      }
      return {
        content: [{ type: "text" as const, text: `Error fetching CVE data: ${response.status} ${response.statusText}` }],
        isError: true,
      };
    }
    ...
    // Extract basic information from the MITRE API response
    const description = data.containers?.cna?.descriptions?.[0]?.value || "No description available.";
    const metrics = data.containers?.cna?.metrics?.[0]?.cvssV3_1 || data.containers?.cna?.metrics?.[0]?.cvssV3_0;
    ...
    return {
      content: [{ type: "text" as const, text: `Failed to fetch CVE data: ${errorMessage}` }],
      isError: true,
    };
  }
}
