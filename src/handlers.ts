...
// In-memory session storage (can be passed in or managed globally, let's keep it global for simplicity matching old behavior, or we can export it)
...
export async function handleAuditItem(args: { sessionId: string; framework: string; itemId: string; status: "pass" | "fail" | "skip"; notes?: string }) {
  ...
  if (!sessions.has(args.sessionId)) {
    sessions.set(args.sessionId, { id: args.sessionId, framework: args.framework, results: [], startedAt: new Date().toISOString() });
  }
...
export async function handleSearchControls(args: { query: string; framework: string }) {
  ...
  const searchIn = args.framework === "all"
...
export async function handleCveLookup(args: { cveId: string }) {
  try {
    ...
    if (!response.ok) {
      ...
      return {
        content: [{ type: "text" as const, text: `Failed to fetch CVE '${args.cveId}'. Status: ${response.status}` }],
        isError: true,
      };
    }
