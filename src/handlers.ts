...
export async function handleGenerateReport({ sessionId, format }: { sessionId: string, format: "json" | "markdown" | "html" }) {
  const session = sessions.get(sessionId);
  if (!session) {
    return {
...
export async function handleCveLookup({ cveId }: { cveId: string }) {
  try {
    const encodedCveId = encodeURIComponent(cveId);
    const response = await fetch(`https://cveawg.mitre.org/api/cve/${encodedCveId}`);
    ...
    if (!response.ok) {
      ...
      return {
        content: [{ type: "text" as const, text: `Error fetching CVE '${cveId}': ${response.statusText}` }],
        isError: true,
      };
    }
    ...
    return {
      content: [{ type: "text" as const, text: `Failed to look up CVE '${cveId}': ${error instanceof Error ? error.message : String(error)}` }],
      isError: true,
    };
  }
}
