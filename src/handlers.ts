...
export async function handleAuditItem(args: { sessionId: string; framework: string; itemId: string; status: "pass" | "fail" | "skip"; notes?: string }) {
  ...
  if (!session) {
    session = { id: sessionId, framework, results: [], startedAt: new Date().toISOString() };
...
export async function handleCveLookup(args: { cveId: string }) {
  ...
  if (!/^CVE-\d{4}-\d{4,}$/.test(cveId)) {
    return {
      content: [{ type: "text" as const, text: `Invalid CVE ID format: ${cveId}` }],
      isError: true,
    };
  }
  ...
  try {
    ...
    return {
      content: [{ type: "text" as const, text: `Failed to fetch CVE data: ${error.message}` }],
      isError: true,
    };
  }
}
