...
export async function handleCveLookup(args: { cveId: string }) {
  try {
    const safeCveId = encodeURIComponent(args.cveId.toUpperCase());
    const response = await fetch(`https://cveawg.mitre.org/api/cve/${safeCveId}`);
    ...
    if (!response.ok) {
      ...
      return {
        content: [{ type: "text" as const, text: `Failed to fetch CVE data. Status: ${response.status}` }],
        isError: true,
      };
    }
