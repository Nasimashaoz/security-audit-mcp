...
export async function handleCveLookup({ cveId }: { cveId: string }) {
  try {
    ...
    if (!response.ok) {
       return {
        content: [{ type: "text" as const, text: `Failed to fetch CVE data: ${response.status} ${response.statusText}` }],
        isError: true,
      };
    }
...
export async function handleGenerateReport({ sessionId, format }: { sessionId: string; format: "json" | "markdown" | "html" }) {
  ...
  if (!fw) {
     return {
        content: [{ type: "text" as const, text: `Framework '${session.framework}' for session not found.` }],
        isError: true,
      };
  }
