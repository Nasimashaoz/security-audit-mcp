...
import { getSession, setSession, hasSession } from "./state.js";
...
export async function handleCveLookup(args: { cveId: string }) {
  const sanitizedCveId = encodeURIComponent(args.cveId);
  try {
    const res = await fetch(`https://cveawg.mitre.org/api/cve/${sanitizedCveId}`);
    if (!res.ok) {
      return {
        content: [{ type: "text" as const, text: `Failed to fetch CVE data for ${args.cveId}: ${res.statusText}` }],
        isError: true,
      };
    }
...
export async function handleAuditItem(args: { sessionId: string; framework: string; itemId: string; status: "pass" | "fail" | "skip"; notes?: string }) {
  ...
  if (!hasSession(args.sessionId)) {
    setSession(args.sessionId, { id: args.sessionId, framework: args.framework, results: [], startedAt: new Date().toISOString() });
  }
