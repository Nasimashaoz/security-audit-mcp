...
export async function handleCveLookup({ cveId }: { cveId: string }) {
  try {
    const sanitizedId = encodeURIComponent(cveId.toUpperCase());
    const response = await fetch(`https://cveawg.mitre.org/api/cve/${sanitizedId}`);
    if (!response.ok) {
       return {
          content: [{ type: "text" as const, text: `Failed to fetch CVE data for ${cveId}: ${response.status} ${response.statusText}` }],
          isError: true,
       }
    }
    ...
    return {
      content: [{ type: "text" as const, text: `Error looking up CVE ${cveId}: ${error.message}` }],
      isError: true,
    }
  }
}
