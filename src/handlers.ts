...
export async function handleCveLookup(args: { cveId: string }) {
  ...
  if (!/^CVE-\d{4}-\d{4,}$/.test(cveId.toUpperCase())) {
    return {
      content: [{ type: "text" as const, text: `Invalid CVE ID format: '${cveId}'. Expected format: CVE-YYYY-NNNN.` }],
      isError: true,
    };
  }

  try {
    const url = `https://cveawg.mitre.org/api/cve/${encodeURIComponent(cveId.toUpperCase())}`;
    const response = await fetch(url);

    if (!response.ok) {
      if (response.status === 404) {
        return {
          content: [{ type: "text" as const, text: `CVE '${cveId}' not found in the MITRE database.` }],
          isError: true,
        };
      }
      return {
    ...
    return {
      content: [{ type: "text" as const, text: `Error looking up CVE: ${errorMessage}` }],
      isError: true,
    };
  }
}
