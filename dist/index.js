...
export const server = new McpServer({
    ...
    version: "1.0.0",
    description: "AI-powered security audit tools for OWASP, NIST, ISO 27001, PCI-DSS, SOC 2, HIPAA, CIS v8, and GDPR",
});
// In-memory session storage
const sessions = new Map();
// ─── Tool: list_frameworks ───────────────────────────────────────────────────
server.tool("list_frameworks", "List all available security audit frameworks", {}, async () => {
...
// ─── Tool: get_framework ─────────────────────────────────────────────────────
server.tool("get_framework", "Get the full checklist for a specific security framework", {
    framework: z.enum(Object.keys(FRAMEWORKS)).describe(`Framework ID: ${Object.keys(FRAMEWORKS).join(" | ")}`),
}, async ({ framework }) => {
...
// ─── Tool: audit_item ────────────────────────────────────────────────────────
server.tool("audit_item", "Record a pass/fail/skip result for a specific audit control item", {
    sessionId: z.string().describe("Unique audit session ID (create any string)"),
    framework: z.enum(Object.keys(FRAMEWORKS)),
    ...
    const fw = FRAMEWORKS[framework];
    const item = fw?.items.find((i) => i.id === itemId);
    if (!item) {
        return {
...
server.tool("generate_report", "Generate a full audit report for a session", {
    sessionId: z.string(),
    ...
    if (format === "markdown") {
        const rows = session.results.map(r => `| ${r.itemId} | ${r.title} | ${r.risk} | ${r.status.toUpperCase()} | ${r.notes} |`).join("\n");
...
server.tool("get_risk_summary", "Get a breakdown of risks by severity level for a framework", {
    framework: z.enum(Object.keys(FRAMEWORKS)),
}, async ({ framework }) => {
    const fw = FRAMEWORKS[framework];
    ...
    fw.items.forEach((item) => {
        if (item.risk in summary) {
            summary[item.risk].push(`${item.id}: ${item.title}`);
        }
    });
    return {
        content: [{
...
server.tool("search_controls", "Search for security controls by keyword across all frameworks", {
    query: z.string().describe("Search term e.g. 'authentication', 'encryption', 'logging'"),
    framework: z.enum(["all", ...Object.keys(FRAMEWORKS)]).default("all"),
    ...
    for (const fwKey of searchIn) {
        ...
        const matches = fw.items.filter((item) => item.id.toLowerCase().includes(q) ||
