...
const searchFrameworkKeys = [...frameworkKeys, "all"];
...
server.tool("get_framework", "Get the full checklist for a specific security framework", {
    framework: z.enum(frameworkKeys).describe(`Framework ID: ${frameworkKeys.join(" | ")}`),
}, async ({ framework }) => {
...
// ─── Tool: cve_lookup ────────────────────────────────────────────────────────
server.tool("cve_lookup", "Look up details for a specific CVE from the official MITRE API", {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            return {
                content: [{ type: "text", text: `Error fetching CVE '${cveId}': ${response.statusText}` }],
                isError: true,
            };
        }
